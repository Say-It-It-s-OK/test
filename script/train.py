...
query_data = load_json_data("data/query_dataset.json")
query_dataset = Dataset.from_dict({
    "text": [item["text"] for item in query_data],
    "labels": [0] * len(query_data)
})

order_data = load_json_data("data/order_dataset.json")
order_dataset = Dataset.from_dict({
    "text": [item["text"] for item in order_data],
    "labels": [1] * len(order_data)
})
